...
public class VersionAdapter21R5 implements VersionAdapter {
    ...
    public Object makeFrameSpawnPacket(final Frame frame) {
        return new PacketPlayOutSpawnEntity(
                ...
                frame.isGlowing() ? EntityTypes.ah : EntityTypes.au,
                switch (frame.getFacing()) {
                    case UP -> 1;
                    case NORTH -> 2;
                    case SOUTH -> 3;
    ...
    public void inject(final Player player, final PacketListener listener, final JavaPlugin plugin) {
        ...
        networkManager.n.pipeline().addBefore("packet_handler", "maps_listener", new PacketHandler21R5(player, listener, plugin));
    }
