...
public class VersionAdapter20R1 implements VersionAdapter {

    private final AtomicInteger entityIdHolder;
    private Field netManField;

    public VersionAdapter20R1() {
    ...
    public void inject(final Player player, final PacketListener listener, final JavaPlugin plugin) {
        ...
        networkManager.m.pipeline().addBefore("packet_handler", "maps_listener", new PacketHandler20R1(player, listener, plugin));
    }
    ...
    public boolean supportsVersion(MinecraftVersion version) {
        return version.greaterThanEquals(MinecraftVersion.RELEASE_1_20)
               && version.lessThanEquals(MinecraftVersion.RELEASE_1_20_1);
    }
}
