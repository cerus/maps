...
public class VersionAdapter19R1 implements VersionAdapter {

    private final AtomicInteger entityIdHolder;

    public VersionAdapter19R1() {
    ...
    public Object makeFrameSpawnPacket(final Frame frame) {
        return new PacketPlayOutSpawnEntity(
                ...
                frame.isGlowing() ? EntityTypes.K : EntityTypes.U,
                switch (frame.getFacing()) {
                    case UP -> 1;
                    case NORTH -> 2;
                    case SOUTH -> 3;
    ...
    public void inject(final Player player, final PacketListener listener, final JavaPlugin plugin) {
        ((CraftPlayer) player).getHandle().b.a().m.pipeline()
                .addBefore("packet_handler", "maps_listener", new PacketHandler19R1(player, listener, plugin));
    }
    ...
    public boolean supportsVersion(MinecraftVersion version) {
        return version.greaterThanEquals(MinecraftVersion.RELEASE_1_19)
               && version.lessThanEquals(MinecraftVersion.RELEASE_1_19_2);
    }
}
